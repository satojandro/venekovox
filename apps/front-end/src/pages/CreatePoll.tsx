...
const BilingualInput: React.FC<BilingualInputProps> = ({ label, placeholder, lang, value = '', onChange }) => (
    <div>
        <label className="block text-sm font-medium text-gray-300 mb-1">{label} ({lang.toUpperCase()})</label>
        <input
            type="text"
    ...
    <div className="bg-gray-900 text-gray-200 min-h-screen font-sans">
      ...
      <main className="container mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="max-w-4xl mx-auto">
          ...
          <form onSubmit={handleSubmit} className="bg-gray-800/50 p-6 sm:p-8 rounded-xl border border-gray-700 space-y-8">
            {/* Poll Title & Description */}
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <BilingualInput label={currentContent.form.pollTitle} placeholder={currentContent.placeholders.title} lang="en" />
              <BilingualInput label={currentContent.form.pollTitle} placeholder={currentContent.placeholders.title} lang="es" />
              <BilingualTextarea label={currentContent.form.description} placeholder={currentContent.placeholders.description} lang="en" />
              <BilingualTextarea label={currentContent.form.description} placeholder={currentContent.placeholders.description} lang="es" />
            </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                    ...
                    <select className="w-full bg-gray-800 border border-gray-600 rounded-md py-2 px-3 text-white focus:outline-none focus:ring-2 focus:ring-blue-500">
                      ...
                      {content.en.topics.map((topic, i) => <option key={topic}>{language === 'en' ? topic : content.es.topics[i]}</option>)}
                    </select>
                  </div>
                  <div>
                    ...
                    <select className="w-full bg-gray-800 border border-gray-600 rounded-md py-2 px-3 text-white focus:outline-none focus:ring-2 focus:ring-blue-500">
                      ...
                      {currentContent.countries.map(country => <option key={country}>{country}</option>)}
                    </select>
                  </div>
                </div>
                {/* Allow Global Participation Toggle */}
                <div className="mt-4">
                    <label className="flex items-center space-x-3 cursor-pointer" onClick={() => setAllowGlobal(!allowGlobal)}>
                        <div className="relative">
                            <input type="checkbox" className="sr-only" checked={allowGlobal} readOnly/>
                            <div className={`block w-10 h-6 rounded-full transition ${allowGlobal ? 'bg-blue-600' : 'bg-gray-600'}`}></div>
                            <div className={`dot absolute left-1 top-1 bg-white w-4 h-4 rounded-full transition-transform ${allowGlobal ? 'translate-x-4' : ''}`}></div>
                        </div>
            ...
            {/* Demographics Toggle */}
            <div>
                <label className="flex items-center space-x-3 cursor-pointer" onClick={() => setShowDemographics(!showDemographics)}>
                    <div className="relative">
            ...
            {/* Voting Options */}
            <div>
              <label className="block text-sm font-medium text-gray-300 mb-2">{currentContent.form.votingOptions}</label>
              <div className="space-y-3">
                {options.map((opt, index) => (
                  <div key={index} className="flex items-center gap-3">
                    <input type="text" placeholder={`${currentContent.placeholders.option} (EN)`} value={opt.en} onChange={(e) => handleOptionChange(index, 'en', e.target.value)} className="w-full bg-gray-800 border border-gray-600 rounded-md py-2 px-3 text-white placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-blue-500"/>
                    <input type="text" placeholder={`${currentContent.placeholders.option} (ES)`} value={opt.es} onChange={(e) => handleOptionChange(index, 'es', e.target.value)} className="w-full bg-gray-800 border border-gray-600 rounded-md py-2 px-3 text-white placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-blue-500"/>
                    <button type="button" onClick={() => handleRemoveOption(index)} className="p-2 text-gray-400 hover:text-red-400"><Trash2 className="w-5 h-5"/></button>
                  </div>
                ))}
              </div>
              <button type="button" onClick={handleAddOption} className="mt-3 flex items-center space-x-2 text-sm text-blue-400 hover:text-blue-300">
                <Plus className="w-4 h-4"/><span>{currentContent.form.addOption}</span>
              </button>
            </div>

             {/* External Resource URL */}
            <div>
                <label className="block text-sm font-medium text-gray-300 mb-1">{currentContent.form.externalResource}</label>
                <input
                    type="url"
            ...
            <div>
                ...
                <div className="flex items-center space-x-4 mb-4">
                    <label className="flex items-center"><input type="radio" name="schedule" value="now" checked={scheduleType === 'now'} onChange={(e) => setScheduleType(e.target.value)} className="mr-2"/>{currentContent.form.openNow}</label>
                    <label className="flex items-center"><input type="radio" name="schedule" value="later" checked={scheduleType === 'later'} onChange={(e) => setScheduleType(e.target.value)} className="mr-2"/>{currentContent.form.setDate}</label>
                </div>
                {scheduleType === 'later' && (
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 p-4 bg-gray-800 rounded-md">
                        <div>
                            <label className="block text-xs text-gray-400 mb-1">Start Date</label>
                            <input type="datetime-local" className="w-full bg-gray-700 border border-gray-600 rounded-md p-2 text-sm"/>
                        </div>
                        <div>
            ...
            <div className="pt-4 border-t border-gray-700">
              <button type="submit" className="w-full bg-blue-600 hover:bg-blue-500 text-white font-bold py-3 px-6 rounded-lg transition duration-300">
