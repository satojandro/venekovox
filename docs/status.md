...
| G05 | Poll screen has mock metadata, eligibility and results; creation is a toast                                                                                                    | **Partial (S3.2)**: `/polls` and `/polls/:id` use one operator descriptor bound to `VITE_*` plus `MACI.getPoll` / `getStartAndEndDate`. Fake totals/eligibility/related polls removed. Creation is still a toast. Poll-0 window remains invalid until a new poll is deployed.                                                                                                                                                                                                         |
