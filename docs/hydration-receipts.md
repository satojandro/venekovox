...
- **Chain verification (G02).** [receiptStatus.ts](../apps/front-end/src/lib/receiptStatus.ts) resolves the configured poll through `MACI.getPoll`, then confirms only a mined successful transaction that emitted `PublishMessage` from **that Poll**, with the participating account in `from`, `to`, or an address-bearing log topic. A successful MACI signup (or any other non-publish tx) is `unexpected`, never `confirmed`. Indirect smart-account execution is handled via logs; full ERC-4337 matching remains W1.
- **Marker retry + distinct states (G03, still partial).** [hydration.ts](../apps/front-end/src/lib/hydration.ts) peeks before any write. Duplicate/in-flight/busy/stale runs cannot blank the display and then skip restore. The hydrated-context marker is set only after participation lookup **and** the receipt check attempt. A unit race harness covers overlapping peek/lookup/receipt/submit/event cases. There is still no React/browser mount harness.
- **Post-submit status.** After `publish` the hook verifies immediately and exposes a guarded `recheckReceipt` for pending/unavailable/unverified/unexpected. Users should not need a reload to leave `unverified`.
